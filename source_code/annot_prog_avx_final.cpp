...
//    cerr << "output:" << (output) << ". idx: " << (idx) << endl;
    ...
    if (feof(frank)){
      cerr << "feof(frank): (" << frank << ") returns: " << (feof(frank)) << " for index: " << (idx) << endl;
  ...
  Reader(int _start, int _finish) {
    ...
    cerr << "Reader (sig" << fsig << ") starting at " << (start) << ". Reached EOF? " << feof(fsig) << endl;
    cerr << "Reader (rank" << frank << "starting at " << (start) << ". Reached EOF? " << feof(frank) << endl;
  }

  void go() {
    PII p;
    cerr << "Before While Loop in Go function" << endl; 
